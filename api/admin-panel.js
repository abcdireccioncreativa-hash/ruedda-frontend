// /api/admin-panel.js
// Devuelve el markup del panel SuperAdmin -- SOLO si quien llama es un
// superadmin real, verificado contra la tabla users con la service_role key.
//
// Por que existe: ese markup vivia como HTML estatico dentro de index.html,
// oculto solo con display:none. Google lo indexaba igual (confirmado: el
// texto aparecia en el snippet de busqueda de ruedda.app) y cualquier
// visitante -- con sesion o sin ella -- podia verlo y ejecutar sus botones
// con solo escribir showView('superadmin') en la consola, sin ningun guard
// real. Ahora el HTML nunca existe en el documento que recibe un visitante
// normal ni un crawler: se pide aparte, y el servidor rechaza a quien no
// pruebe ser superadmin con un JWT valido.
//
// Mismo patron que admin-user.js / mod-listing.js: getUserFromToken +
// isSuperadmin(), que ya existen en lib/supabase.js -- no se duplica logica
// de auth nueva.
//
// GET, header Authorization: Bearer <jwt>. Responde texto/html si autoriza,
// 401/403 con JSON {error} si no.

const { getUserFromToken, isSuperadmin } = require('../lib/supabase');
...
module.exports = async (req, res) => {
  ...
  // nunca cachear: es contenido sensible y condicionado a la sesion de quien pide
  res.setHeader('Cache-Control', 'no-store, private');
  res.status(200).send(HTML);
};

const HTML = `<div id="view-superadmin" class="view">
