...
// Versión "carta": escrita por una persona, sin diseño de campaña, un solo
// link. Gmail la trata como correo personal y no como promoción.
function letterLayout({ nombre }) {
  const first = String(nombre || '').trim().split(/\s+/)[0];
  const hola = first ? 'Hola ' + esc(first) + ',' : 'Hola,';
  const p = (t) => `<p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#222">${t}</p>`;
  return `<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Ruedda en tu iPhone</title></head>
...
${p(hola)}
${p('Te escribo para contarte algo que llevábamos tiempo esperando: Ruedda ya está en la App Store.')}
${p('Todo lo que ya conoces, el market, las subastas y los concesionarios, ahora está en una app hecha para iPhone. Es más rápida, te avisa al momento cuando alguien te supera en una subasta o te escribe, y entras con tu mismo usuario o correo de siempre.')}
${p('La puedes descargar aquí:<br><a href="' + APP_STORE_URL + '" style="color:#0066cc">Ruedda en la App Store</a>')}
${p('Si tienes cualquier duda o algo no te funciona, respóndeme este correo y lo vemos.')}
${p('Gracias por estar con nosotros desde el principio.')}
<p style="margin:24px 0 0;font-size:15px;line-height:1.5;color:#222">Enrique<br><span style="color:#777">Ruedda</span></p>
<p style="margin:36px 0 0;font-size:11.5px;line-height:1.6;color:#aaa">Si no quieres recibir más correos como este, <a href="${UNSUB_URL}" style="color:#aaa">avísanos aquí</a>.</p>
</div>
</body></html>`;
}

function letterText({ nombre }) {
  const first = String(nombre || '').trim().split(/\s+/)[0];
  return [
    first ? 'Hola ' + first + ',' : 'Hola,',
    '',
    'Te escribo para contarte algo que llevábamos tiempo esperando: Ruedda ya está en la App Store.',
    '',
    'Todo lo que ya conoces, el market, las subastas y los concesionarios, ahora está en una app hecha para iPhone. Es más rápida, te avisa al momento cuando alguien te supera en una subasta o te escribe, y entras con tu mismo usuario o correo de siempre.',
    '',
    'La puedes descargar aquí:',
    APP_STORE_URL,
    '',
    'Si tienes cualquier duda o algo no te funciona, respóndeme este correo y lo vemos.',
    '',
    ...
    'Ruedda',
    '',
    'Si no quieres recibir más correos como este, escríbenos a contactoruedda@gmail.com.'
  ].join('\n');
}
...
const templates = {
  ...
  appDisponibleCarta: ({ nombre } = {}) => {
    const first = String(nombre || '').trim().split(/\s+/)[0];
    return {
      subject: first ? first + ', ya puedes descargar Ruedda en tu iPhone' : 'Ya puedes descargar Ruedda en tu iPhone',
      html: letterLayout({ nombre }),
      text: letterText({ nombre }),
      from: 'Enrique de Ruedda <enrique@ruedda.app>',
      replyTo: 'contactoruedda@gmail.com'
    };
  },
